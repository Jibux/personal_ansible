...
def backup_to_nas(item: dict, dry_run: bool, verbose: bool) -> None:
    ...
    nas_dest = item.get("nas_destination")
...
def backup_to_cloud(item: dict, dry_run: bool, verbose: bool) -> None:
    name = item.get("name", "<unnamed>")
    source = item.get("source")
    cloud_dest = item.get("cloud_destination")
...
def write_backup_list(items: list[dict], backup_list_path: Path, dry_run: bool, verbose: bool) -> None:
    lines = []
    for item in items:
        nas_src = item.get("nas_source")
        cloud_dest_from_nas = item.get("cloud_destination_from_nas") or item.get("cloud_destination")
...
def main() -> None:
    ...
    parser.add_argument(
        "--names", default=[], action=SplitArgs, help="Comma-separated list of backup names to run")
